...
class e2sm_rc_module(object):
    ...
    def send_control_request_style_1_action_2(self, e2_node_id, ue_id, drb_id=1, qfi=1, direction=0, ack_request=1):
        """E2SM-RC Radio Bearer Control / QoS flow mapping (action 2).

        This is the only CONTROL action implemented by the OAI gNB agent in this lab
        (ran_func_rc.c asserts ctrl_act_id == 2). Do NOT use action 6 (PRB quota).
        """
        if int(direction) not in (0, 1):
            raise ValueError("direction must be 0 (UL) or 1 (DL)")
        if not (0 <= int(qfi) <= 64):
            raise ValueError("qfi out of range")

        ue_id_tuple = ('gNB-DU-UEID', {'gNB-CU-UE-F1AP-ID': int(ue_id)})
        control_header = self.e2sm_rc_compiler.pack_ric_control_header_f1(
            style_type=1, control_action_id=2, ue_id_tuple=ue_id_tuple
        )
        control_msg_dict = {
            'ric-controlMessage-formats': (
                'controlMessage-Format1',
                {
                    'ranP-List': [
                        # DRB ID (ELEMENT, key flag true)
                        {
                            'ranParameter-ID': 1,
                            'ranParameter-valueType': (
                                'ranP-Choice-ElementTrue',
                                {'ranParameter-value': ('valueInt', int(drb_id))},
                            ),
                        },
    ...
    control_qos_flow_mapping = send_control_request_style_1_action_2
