...
## Loop fechado (emulação + atuação real)

O elo policy → atuação → evidência KPM está em
[FASE3_CLOSED_LOOP.md](FASE3_CLOSED_LOOP.md):
...
- **emulate:** `tc` em `oaitun_ue*` (efeito mensurável em `DRB.UEThpUl`).
- **real:** E2SM-RC action **2** (QoS flow mapping) — único CONTROL seguro no gNB OAI
  deste lab. Action 6 (PRB quota) é recusada (abortaria o softmodem).

## Evolução recomendada

1. Versionar datasets JSONL por `run_id` e comparar MAD vs modelos supervisionados.
2. Publicar um policy type A1 dedicado no Mediator (além do type `1` de lab).
3. Ampliar o PoC OAI de CONTROL se/quando o agente aceitar mais actions sem assert.
4. Adicionar autenticação/TLS antes de sair do host de laboratório.

Policies A1 expressam intenção/limites; a ação de baixa latência continua no
xApp/atuador via E2 (ou emulação controlada). Esse limite preserva a separação
nonRT/nearRT da arquitetura O-RAN.
