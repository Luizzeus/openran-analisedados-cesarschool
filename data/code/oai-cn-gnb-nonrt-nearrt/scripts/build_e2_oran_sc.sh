...
# Preservar binário FlexRIC (porta 36421) — o build OAI sobrescreve ran_build/build/.
FLEXRIC_SOFTMODEM_BAK="$ORAN_BIN_DIR/nr-softmodem-flexric.bak"
if [ -f "$RAN_BUILD/nr-softmodem" ]; then
    cp -f "$RAN_BUILD/nr-softmodem" "$FLEXRIC_SOFTMODEM_BAK"
    echo "Backup FlexRIC: $FLEXRIC_SOFTMODEM_BAK"
else
    echo "AVISO: $RAN_BUILD/nr-softmodem ausente — rode ./scripts/build_e2.sh depois deste build."
fi
...
restore_after_build() {
    ...
    # Restaurar gNB FlexRIC para Fase 1 (porta 36421)
    if [ -f "$FLEXRIC_SOFTMODEM_BAK" ]; then
        cp -f "$FLEXRIC_SOFTMODEM_BAK" "$RAN_BUILD/nr-softmodem"
        echo "Restaurado nr-softmodem FlexRIC (:$E2_PORT_FLEX) em $RAN_BUILD/"
    fi
}
...
echo "  FlexRIC será restaurado em: $RAN_BUILD/nr-softmodem (E2 → :$E2_PORT_FLEX)"
echo ""
