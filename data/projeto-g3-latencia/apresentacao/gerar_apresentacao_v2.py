# -*- coding: utf-8 -*-
"""
Gera a apresentação FINAL do seminário do Projeto Integrador G3 — Latência / proxy de QoE.

Versão 2 (finalização): acrescenta um modelo comparativo pesquisado — o controle
estatístico EWMA (média móvel exponencialmente ponderada, família D do documento
`analise-modelos-alternativos.md`) — como alternativa ao gatilho de persistência
"janela de 5 fixa". Traz dois gráficos novos (carta de controle EWMA e barras
comparativas por fase) e o texto descritivo correspondente, além da referência
cruzada ao modelo do docente (`robust-baseline-mad`).

Saída:
    apresentacao/figuras/*.png             — gráficos e diagramas (gerados do dataset)
    apresentacao/seminario-g3-latencia-v2.pdf

Reproduz: `python gerar_apresentacao_v2.py` (na pasta apresentacao/).
Fonte única de dados: ../../code/datasets/kpm-ue-tp-sample/{kpm.jsonl, model.json, decision.json}
"""
import json
...
MODELJSON = HERE / "../../code/datasets/kpm-ue-tp-sample/model.json"
PDF_OUT = HERE / "seminario-g3-latencia-v2.pdf"
...
# cores dos modelos no comparativo
M_DOCENTE = "#7A5195"   # robust-baseline-mad (docente)
M_GRUPO   = ACCENT      # limiar empírico + zona morta (grupo)
M_EWMA    = "#EF6C00"   # EWMA (modelo comparativo pesquisado)
...
# ----------------------------------------------------------------------------
# MODELO COMPARATIVO 1 — EWMA (família D do catálogo de alternativas)
#   z_i = lam * x_i + (1-lam) * z_{i-1};  z_0 = mediana(baseline bruto)
#   sigma robusto = 1.4826 * MAD(baseline ativo);  UCL = z_0 + 3 sigma sqrt(lam/(2-lam))
#   parâmetros clássicos, sem tuning (Roberts 1959)
# ----------------------------------------------------------------------------
LAM = 0.3
seq = df["delay"].to_numpy(float)
med_a = float(np.median(arr_a["baseline"]))
mad_a = float(np.median(np.abs(arr_a["baseline"] - med_a)))
sigma_r = 1.4826 * mad_a                                  # ~69.7 µs
z_ewma = np.empty(len(seq))
z_ewma[0] = float(np.median(arr["baseline"]))             # 0.0
for i in range(1, len(seq)):
    z_ewma[i] = LAM * seq[i] + (1 - LAM) * z_ewma[i - 1]
UCL = z_ewma[0] + 3 * sigma_r * np.sqrt(LAM / (2 - LAM))  # ~87.8 µs
ewma_alarm = z_ewma > UCL
ewma_by_phase = {p: round(100 * float(ewma_alarm[df.phase.to_numpy() == p].mean()), 1) for p in PH}

def _maior_corrida(mask):
    best = cur = 0
    for v in mask:
        cur = cur + 1 if v else 0
        best = max(best, cur)
    return best
ewma_run_stress = _maior_corrida(ewma_alarm[df.phase.to_numpy() == "stress"])

# ----------------------------------------------------------------------------
# MODELO COMPARATIVO 2 (referência) — robust-baseline-mad do docente
#   score(x,f) = |x - mediana_f| / max(MAD_f, mad_floor);  apply se >= 2 features anômalas
# ----------------------------------------------------------------------------
MODEL = json.loads(MODELJSON.read_text(encoding="utf-8"))
_COLMAP = {"DRB.RlcSduDelayDl": "delay", "DRB.UEThpUl": "thp", "RRU.PrbTotUl": "prb"}
_mf, _thr, _minf = MODEL["mad_floor"], MODEL["score_threshold"], MODEL["min_anomalous_features"]
_flags = []
for feat, prm in MODEL["features"].items():
    col = _COLMAP[feat]
    esc = max(prm["mad"], _mf)
    _flags.append(((df[col] - prm["median"]).abs() / esc > _thr).to_numpy())
docente_apply = np.sum(_flags, axis=0) >= _minf
docente_by_phase = {p: round(100 * float(docente_apply[df.phase.to_numpy() == p].mean()), 1) for p in PH}

# limiar do grupo, nível de amostra (delay > L), por fase — para o comparativo
grupo_by_phase = {p: frac_above(arr[p], L) for p in PH}
...
NUM = dict(L=L, GAP_LO=GAP_LO, GAP_HI=GAP_HI, kpi1_bruto=kpi1_bruto, kpi1_ativo=kpi1_ativo,
           ...
           LAM=LAM, sigma_r=sigma_r, UCL=UCL, ewma_by_phase=ewma_by_phase,
           ewma_run_stress=ewma_run_stress, docente_by_phase=docente_by_phase,
           grupo_by_phase=grupo_by_phase)
...
# 13. NOVO — carta de controle EWMA sobre as 100 amostras
def fig_ewma():
    fig, ax = plt.subplots(figsize=(11.4, 4.6))
    # atraso bruto ao fundo, para referência
    ax.plot(df.gidx, seq, color=MUTED, lw=0.9, alpha=0.35, label="DRB.RlcSduDelayDl (bruto)")
    # bandas de fase
    for p in PH:
        g = df[df.phase == p]
        ax.axvspan(g.gidx.min(), g.gidx.max() + 1, color=PHASE_C[p], alpha=0.06, lw=0)
    # estatística EWMA
    ax.plot(df.gidx, z_ewma, color=M_EWMA, lw=2.1, label=f"EWMA  (λ = {LAM})")
    below = ~ewma_alarm
    ax.scatter(df.gidx[below], z_ewma[below], s=18, color=M_EWMA, zorder=3)
    ax.scatter(df.gidx[ewma_alarm], z_ewma[ewma_alarm], s=42, color=STRESS_C, zorder=4,
               label="EWMA acima do limite de controle (alarme)")
    ax.axhline(UCL, color=INK, ls="--", lw=1.4)
    ax.text(2, 300, f"UCL = {UCL:.1f} µs\nz₀ + 3σ·√(λ/(2−λ)),  σ = {sigma_r:.0f} µs robusto",
            color=INK, fontsize=10, va="top")
    ax.annotate("", xy=(6, UCL), xytext=(6, 285),
                arrowprops=dict(arrowstyle="-", color=INK, lw=0.8))
    ax.set_xlim(0, len(df)); ax.set_ylim(-10, max(seq.max(), z_ewma.max()) * 1.06)
    ax.set_xlabel("amostra (ordem: baseline - stress - recovery)")
    ax.set_ylabel("µs")
    _finish(ax); ax.legend(frameon=False, fontsize=10, loc="upper right")
    save(fig, "ewma.png")

# 14. NOVO — comparativo dos três modelos: % de amostras sinalizadas por fase
def fig_compara():
    fig, ax = plt.subplots(figsize=(10.6, 4.7))
    x = np.arange(3); w = 0.26
    dv = [docente_by_phase[p] for p in PH]
    gv = [grupo_by_phase[p] for p in PH]
    ev = [ewma_by_phase[p] for p in PH]
    b1 = ax.bar(x - w, dv, w, color=M_DOCENTE,
                label="Modelo do docente — robust-baseline-mad (apply ≥ 2 features)")
    b2 = ax.bar(x,     gv, w, color=M_GRUPO,
                label=f"Grupo — limiar empírico (delay > L = {L:.1f} µs)")
    b3 = ax.bar(x + w, ev, w, color=M_EWMA,
                label=f"Comparativo — EWMA (λ = {LAM}, 3σ)")
    for bars in (b1, b2, b3):
        ax.bar_label(bars, fmt="%.0f%%", fontsize=9.5, color=INK, padding=2)
    ax.axvspan(1.55, 2.45, color=REC_C, alpha=0.06, lw=0)
    ax.text(2, 70, "só a EWMA enxerga\na cauda de recovery", ha="center", color=REC_C, fontsize=9.5)
    ax.set_xticks(x); ax.set_xticklabels([p for p in PH])
    ax.set_ylabel("% de amostras da fase sinalizadas"); ax.set_ylim(0, 118)
    _finish(ax)
    ax.legend(frameon=False, fontsize=9, loc="lower center", bbox_to_anchor=(0.5, 1.0))
    save(fig, "compara.png")
...
c.setTitle("Seminário G3 — Latência / proxy de QoE (v2, com modelo comparativo)")
...
text(MX, 190, "Seminário — Aula 06 · 29/08/2026  ·  versão final: inclui modelo comparativo (EWMA)  ·  dados: lab OAI RFSIM · 100 amostras",
     F_IT, 10.5, MUTED)
...
# ---- 15 NOVO — modelo comparativo: EWMA ----
new_page("Modelo comparativo", "EWMA — controle estatístico como gatilho de persistência")
yc = para(MX + 16, CT,
     "Do catálogo de modelos pesquisados (analise-modelos-alternativos.md, família D): a média móvel "
     "exponencialmente ponderada substitui a \"janela de 5 fixa\" por um limite de controle estatístico, "
     "sem ajuste fino.",
     F_REG, 12.5, INK, leading=18, width_chars=124)
text(MX + 16, yc - 4,
     f"z_i = λ·x_i + (1−λ)·z_(i−1),  λ = {LAM};  σ robusto = 1,4826·MAD(baseline ativo) ≈ {sigma_r:.0f} µs;  "
     f"UCL ≈ {UCL:.1f} µs.",
     F_IT, 10.5, MUTED)
img(FIG / "ewma.png", MX + 8, yc - 26, PW - 2 * MX - 16, 268, align="c")
text(MX + 16, 60,
     f"Alarme por fase: baseline {ewma_by_phase['baseline']:.0f} % · stress {ewma_by_phase['stress']:.0f} % · "
     f"recovery {ewma_by_phase['recovery']:.0f} %.  Corrida contínua de alarme em stress: {ewma_run_stress} amostras.",
     F_REG, 11.5, INK)

# ---- 16 NOVO — comparativo dos três modelos ----
new_page("Comparativo", "Três modelos, a mesma decisão de fase — por caminhos diferentes")
img(FIG / "compara.png", MX + 8, CT, 470, 292)
bullets(PW - MX - 350, CT + 4, [
    "Os três concordam: stress é a fase crítica (≈ 100 % das amostras sinalizadas).",
    ...
    "Grupo (limiar em µs): mede a variável certa; unidade física audível; reporta a incerteza do ponto.",
    "EWMA: único que acende na cauda de recovery (55 %) — expõe a recuperação parcial que os outros dois perdem.",
], width_chars=44, gap=30, size=11.5)
text(MX + 16, 58,
     "Veredito: manter o limiar do grupo como \"nível\"; adotar EWMA como \"persistência\" (ARL controlado, sem tuning); "
     "modelo do docente serve de verificação cruzada.",
     F_IT, 10.5, MUTED)
...
], gap=22, size=12.4, width_chars=100)
...
], gap=26, size=12.8, width_chars=100)
...
text(MX + 44, 90, "Este PDF é gerado por apresentacao/gerar_apresentacao_v2.py a partir do mesmo dataset.", F_IT, 10.5, MUTED)
footer(); c.showPage()
c.save()
