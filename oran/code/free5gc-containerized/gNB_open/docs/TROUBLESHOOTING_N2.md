...
- **`du-zmq-srsue.yml`** `ru_sdr.device_args` tem de usar as **mesmas portas TCP** que **`ue_srsue.conf`**, e o `docker-compose` tem de publicar o **DL** (ex. `2002:2002` se DL=2002). Ver **`configs/ZMQ_PORTS.md`** (gNB_open **2002/2003** vs monolítico **2000/2001**).
- **`start-du-after-ue.sh`** espera a porta **UL** correta (`UE_ZMQ_UL_PORT`, defeito **2003** para gNB_open). Se ainda apontar para **2001**, o script nunca deteta o UE ou deteta o srsUE do monolítico → **Attaching…** ou comportamento errático.
...
O **CU** neste repositório deve ter **`10.100.200.51`** em `eth0` (ver `gNB_open/docker-compose.yaml`). Nos logs do AMF, o contexto RAN deve referir **esse** IP.
...
- Execute: `./scripts/verify-ran-net.sh` (a partir de `gNB_open/`) e confira a saída.
...
```bash
# Com o CU a correr
ls -la gNB_open/logs/cu_ngap.pcap
```
